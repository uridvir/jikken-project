...
JikkenFrame::JikkenFrame() : wxFrame(nullptr, wxID_ANY, L"じっけん", wxPoint(50, 50), wxSize(800, 600)) {
    wxMenu* menuFile = new wxMenu();
    menuFile->Append(ID_PROPERTIES, L"&プロパティ\tCtrl-P", L"自動ダウンロード、バブル分析オプション、など．．．");
    menuFile->AppendSeparator();
    menuFile->Append(wxID_EXIT, L"出る");  // TODO: Add status bar text in Japanese

    wxMenu* menuBubble = new wxMenu();
    menuBubble->Append(ID_BUBBLE, L"ファイルを開く\tCtrl-B", L"ファイルと、アルゴリズムがバブル分析をします。");

    wxMenu* menuHelp = new wxMenu();
    menuHelp->Append(wxID_ABOUT, L"じっけんプログラム事", L"もっとじっけんプログラムの方法を教えられます。");
...
void JikkenFrame::OnExit(wxCommandEvent& event) {
    ...
    const std::string downloadAvailable =
        "もうダウンロード出来るビデオがありますよ！";  // Video that can be downloaded already exists!
    const std::string currentDownload =
        "今こそ、プログラムがビデオをダウンロードしています！";                // Now, the program is downloading video!
...
void JikkenFrame::OnAbout(wxCommandEvent& event) {
    wxMessageBox(
        L"じっけんプログラムことは、WPI大学たの四年生プロジェクトのため出に来ました。"
...
void JikkenFrame::OnBubble(wxCommandEvent& event) {
    std::thread([this]() {
        wxFileDialog openPrompt(this, L"バブル分析ためビデオ開", "", "", "ビデオファイル (*.avi;*.mp4)|*.avi;*.mp4",
